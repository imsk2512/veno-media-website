...
export function Portfolio() {
  return (
    <section id="portfolio" className="relative scroll-mt-24 bg-white py-24 sm:py-28">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((project, i) => (
            <Reveal key={project.title} delay={(i % 3) * 80} className={project.span}>
              <article className="group relative h-72 overflow-hidden rounded-[28px] border border-border bg-primary shadow-soft sm:h-80">
                <Image
                  src={project.image}
                ...
                <span className="absolute right-5 top-5 inline-flex h-11 w-11 items-center justify-center rounded-full bg-white/15 text-white opacity-0 backdrop-blur-md transition-all duration-300 group-hover:opacity-100">
